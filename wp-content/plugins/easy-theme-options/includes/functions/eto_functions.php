		...
		echo	'<td>';
				...
				switch($field['type']) {
					...
					case 'repeatable':
						...
						$i = 0;
						// foreach ($field['repeatable_fields'] as $row) {
						// 	print_r($row);
						...
						if ( $eto_options['support_name'] ) {
							...
							foreach($eto_options['support_name'] as $key=>$row) {
								echo '<li><span class="sort hndle"><img src="' . ETO_PLUGIN_DIR . 'includes/images/cursor_move.png" /></span>
											<input type="text" name="eto_settings['.$field['id1'].']['.$i.']" id="eto_settings['.$field['id1'].']" value="'.$row.'" size="30" />
											<input type="text" name="eto_settings['.$field['id2'].']['.$i.']" id="eto_settings['.$field['id2'].']" value="'.$eto_col2[$key].'" size="30" />
											<a class="repeatable-remove button" href="#">'.__('Delete','eto').'</a></li>';
								$i++;
							}
						} else {
							echo '<li><span class="sort hndle">|||</span>
										...
										<input type="text" name="eto_settings['.$field['id1'].']['.$i.']" id="eto_settings['.$field['id1'].']" value="" size="30" />
										<label>SĐT hổ trợ	</label>
										<input type="text" name="eto_settings['.$field['id2'].']['.$i.']" id="eto_settings['.$field['id2'].']" value="" size="30" />
										<a class="repeatable-remove button" href="#">'.__('Delete','eto').'</a></li>
									';
						}
						echo '</ul>';
